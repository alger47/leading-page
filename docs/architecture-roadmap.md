...
| 7 | ~~Phase 13 performance~~ | ✅ Implemented (2026-09-17): `pnpm perf` harness (`scripts/perf.mjs`) — per-envelope `renderMs` (react-dom/server), published-page vitals over a real file:// Chrome navigation (LCP/CLS/load), image report (format/dims/bytes), §5.7 budget gate (LCP < 2.5 s, CLS < 0.1; breach → exit 1, Chrome-less → skip-not-fail), and surfaces golden per-stage generation latency p50/p95 + e2e + cost from the engine reports. `packages/visual-qa` gained `measureVitals()`/`withinBudgets()` (6 new tests, package 21/21). Measured 2026-09-17: render 3–20 ms, LCP 108–328 ms, CLS 0–0.078 — within all §5.7 budgets. `qa/perf-baseline.json` records the run. |
| 8 | ~~Phase 14 Security (PART XII full pass)~~ | ✅ Implemented (2026-09-17): **IDOR** — `tests/security.integration.test.ts` asserts cross-tenant 404 across every page/project/job route (read + all mutations, publish/unpublish/versions/restore/regenerate/generate). **CSRF** — 403 matrix across all 9 mutating endpoints. **XSS** — `packages/page-schema/src/schemes.ts` (`isSafeHref`/`sanitizeHref`, scheme allowlist http/https/mailto/tel/#/relative/asset:) wired into `validateStructural` (new `E-VAL-STRUCT-004`, gates every save/publish) + renderer guard in `Button`/`Header`/`Footer` + placeholder-SVG label escaping in `apps/web/lib/assets.ts`; section schemas document the scheme pattern. **Secrets/ops** — gitleaks CI job; `healthz` no longer probes process/PID-1 env presence; fail-closed prod guards (`assertProdConfig` in worker bootstrap, per-request throw in `GenerationService` when `WORKER_INTERNAL_TOKEN` is the dev default); storage `put` MIME allowlist (images only). **Headers** — COOP/CORP/OAC added + CSP documented (kept `unsafe-inline`/`unsafe-eval` for Next bootstrap, XSS closed at source); introspection test. `docs/security.md` documents the §12.1 threat matrix. |
| 9 | ~~Phase 15 Production readiness & handover~~ | ✅ Implemented (2026-09-17): **Deployment** — `docker-compose.yml` (db/cache/migrate/engine/worker/web, one-shot `prisma migrate deploy` gating web+worker) + `apps/*/Dockerfile` + `.dockerignore` + `docker-compose.env.example`, alongside the existing `render.yaml` platform path; `docker compose config` validated with the official Compose v5.5.1 binary + Dockerfiles statically audited, but images not built here (no daemon) so `docker compose build && up` is a pre-deploy gate. **HTTPS** — HSTS header added + tested. **Cookies** — flags centralized in `lib/api.ts` (`sessionCookieFlags`/`csrfCookieFlags`), `webConfig().secureCookies` (`COOKIE_SECURE` override), prod `Secure` tested. **CORS** — same-origin by design (no headers, documented). **Env** — root + web `.env.example` corrected/complete (dead names removed). **Handover** — `docs/production-readiness.md` + `docs/runbook.md` + `ADR-0011`; walkthrough debt (Phases 12–14) paid. |
