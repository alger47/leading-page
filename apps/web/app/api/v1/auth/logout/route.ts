...
export async function POST(request: NextRequest) {
  try {
    ...
    res.cookies.set(config.sessionCookieName, '', { httpOnly: true, sameSite: 'strict', path: '/', maxAge: 0 });
    res.cookies.set(config.csrfCookieName, '', { httpOnly: false, sameSite: 'strict', path: '/', maxAge: 0 });
    return res;
  } catch (error) {
